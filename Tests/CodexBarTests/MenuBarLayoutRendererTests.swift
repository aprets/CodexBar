...
struct MenuBarLayoutRendererTests {
    ...
    @Test
    func `Codex percentage tokens show every account weekly limit`() {
        let renderer = MenuBarLayoutRenderer()
        ...
        let data = self.data(accountWeeklyWindows: accountWeeklyWindows)

        for token in [
            MenuBarLayoutToken.percent(window: .automatic),
            MenuBarLayoutToken.percent(window: .weekly),
        ] {
            let used = renderer.render(
                layout: MenuBarLayout(lines: [[token]]),
                data: data,
                icon: nil,
                options: self.options())
            ...
            #expect(used.attributedTitle.string == "60% / 25%")
            #expect(used.accessibilityLabel == L("%@ %@", L("Weekly"), "60%, 25%"))
            #expect(remaining.attributedTitle.string == "40% / 75%")
        }

        let singleAccount = renderer.render(
            layout: MenuBarLayout(lines: [[.percent(window: .automatic)]]),
            data: self.data(accountWeeklyWindows: [accountWeeklyWindows[0]]),
            icon: nil,
            options: self.options())
        let otherProvider = renderer.render(
            layout: MenuBarLayout(lines: [[.percent(window: .automatic)]]),
            data: self.data(provider: .claude, accountWeeklyWindows: accountWeeklyWindows),
            icon: nil,
            options: self.options())

        #expect(singleAccount.attributedTitle.string == "50%")
        #expect(otherProvider.attributedTitle.string == "50%")
    }
    ...
    private func data(
        ...
        accountWeeklyWindows: [MenuBarLayoutRenderWindow?] = [],
        ...
        -> MenuBarLayoutRenderData
    {
        MenuBarLayoutRenderData(
            ...
            providerName: "Codex",
            accountLabel: "user@example.com",
            accountWeeklyWindows: accountWeeklyWindows,
