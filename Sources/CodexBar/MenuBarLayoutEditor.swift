...
struct MenuBarLayoutPreview: View {
    ...
    func liveData(provider: UsageProvider, snapshot: UsageSnapshot) -> MenuBarLayoutRenderData {
        ...
        // Provider-specific by design: only Codex exposes reconciled visible-account snapshots.
        let accountWeeklyWindows: [MenuBarLayoutRenderWindow?] = if provider == .codex {
            (self.store.codexAccountSnapshots.filter(\.account.isActive)
                + self.store.codexAccountSnapshots.filter { !$0.account.isActive }).map { accountSnapshot in
    ...
    private func representativeData(provider: UsageProvider) -> MenuBarLayoutRenderData {
        ...
        // Provider-specific by design: the Codex preview mirrors its two-account menu-bar output.
        let accountWeeklyWindows: [MenuBarLayoutRenderWindow?] = provider == .codex
            ? [
                MenuBarLayoutRenderWindow(weekly),
